...
public class PaymentHandler {
    ...
    public synchronized TransactionSuccessful makePayment(Transaction tx) throws AccountException, PaymentFailedException {
        ...
        Optional<Account> payer = cache.getOptionalAccount(tx.getPayer());
        Optional<Account> payee = cache.getOptionalAccount(tx.getPayee());

        if(payee.isPresent() == false)
            throw new AccountException("payee account not found");
        if(payer.isPresent() == false)
            throw new AccountException("payer account not found");
